...
>> 즉, Ajax는 비동기 통신 방식이지만 결국 HTTP 프로토콜을 사용하기 때문에 StateLess합니다.
...
>> WebSocket은 StateFul 합니다. 따라서 요청을 보내지 않아도 응답을 받을 수 있습니다.
>> 작동 방식을 간단하게 설명해보겠습니다.
>>
>> 채팅창에 참여하면 웹 소켓이 만들어집니다.
>> 그 후 채팅을 보낼 때 웹 소켓에 특정 신호를 보내고, 동시에 그 신호를 해당 웹 소켓 세션에 접속해 있는 모든 클라이언트들에게 뿌립니다.
>> 웹 소켓에서 신호를 받게 되면 채팅창을 갱신해줍니다.
>> 결과적으로 채팅을 보내거나 받을 때만 채팅창이 갱신되기 때문에 서버 부하를 줄이고 사용자 경험을 향상 시킨 실시간 통신이 이루어지게 됩니다.
>>
> 4. 트러블 슈터
>> 팀장 역할을 맡고 가장 많이 들었던 멘트는 "승연씨 이 부분이 잘 안되는데, 시간 나면 한번만 봐주세요" 였습니다.
